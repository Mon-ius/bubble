...
tradition, and the emerging work on large language models as
economic~agents.
...
\paragraph{Expected utility and LLM traders.}
\citet{lopezlira2025llm} introduces a utility-maximising LLM agent
whose prior is a perturbation of~$\FV_t$, whose action space is
...
\paragraph{Language models as economic subjects.}
A small but rapidly growing literature studies whether large language
models reproduce well-documented behavioural regularities when
embedded in experimental decision tasks. The Wang~2026 paradigm is a
deliberately narrow use of this approach. It does not ask the
language model to trade; it does not expose the order book or the
history of transaction prices; it does not grant the model the
authority to choose an action from the action set. It asks one
question---``what is your private subjective valuation per share
right now, in cents?''---with a prompt that names the agent's risk
preference, bias mode, and the composition of the market. The
returned number is then clamped and treated as a prior in the
downstream expected-utility computation. This design isolates the
psychological channel (``how would the model price the asset, in
light of who it is?'') from the trading channel (``what orders
would the model place?''), and in doing so connects the experimental
bubble literature to the psychology-of-valuation
literature via a single numeric quantity.
...
the quantitative form of the Wang~2026 research question: when
